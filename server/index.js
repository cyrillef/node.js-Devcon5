...
var Unirest = function (method, uri, headers, body, callback) {
  var unirest = function (uri, headers, body, callback) {
    var $this = {
	  ...
	  /**
       * Set HTTP Request progress callback.
       * Upon HTTP Response invokes `callback` with a two argument, the uncomplete `[Response](#response)` object, and the chunck data.
       *
       ...
       */
        ...
        function handleGZIPResponse (response) {
			response.on('data', function (chunk) {
				($this.progressCB) && $this.progressCB(parseInt(response.headers['content-length'], 10), chunk);
			});
        ...
        Request.on('response', handleGZIPResponse);
		  Request.on('data', function (chunk) {
			  ($this.progressCB) && $this.progressCB(parseInt(this.response.headers['content-length'], 10), chunk);
		  });
